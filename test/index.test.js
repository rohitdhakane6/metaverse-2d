...
axios2.defaults.baseURL = BACKEND_URL;
...
describe("Authentication", () => {
  test("User is able to sign up only once", async () => {
    const username = `rohit${Math.random()}`; // rohit0.12331313
    ...
    const updatedResponse = await axios.post("/api/v1/signup", {
      username,
      password,
      type: "admin",
    });
  ...
  test("Signup request fails if the username is empty", async () => {
    const username = `rohit-${Math.random()}`; // rohit-0.12312313
    const password = "123456";
  ...
  test("Signin succeeds if the username and password are correct", async () => {
    const username = `rohit-${Math.random()}`;
    const password = "123456";
...
describe("User avatar information", () => {
  ...
  beforeAll(async () => {
    ...
    const signupResponse = await axios.post("/api/v1/signup", {
      username,
      password,
      type: "admin",
    });
  ...
  test("Available avatars lists the recently created avatar", async () => {
    const response = await axios.get("/api/v1/avatars");
...
describe("Space information", () => {
  ...
  beforeAll(async () => {
    ...
    const userSignupResponse = await axios.post("/api/v1/signup", {
    ...
    const userSigninResponse = await axios.post("/api/v1/signin", {
      username: `${username}-user`,
      password,
    });
  ...
  test("User is not able to delete a space that doesnt exist", async () => {
    const response = await axios.delete("/api/v1/space/randomIdDoesntExist", {
      headers: {
        authorization: `Bearer ${userToken}`,
      },
    });
  ...
  test("User is able to delete a space that does exist", async () => {
    ...
    const deleteReponse = await axios.delete(
      `/api/v1/space/${response.data.spaceId}`,
      {
        headers: {
          authorization: `Bearer ${userToken}`,
        },
...
describe("Arena endpoints", () => {
  ...
  test("Incorrect spaceId returns a 400", async () => {
    const response = await axios.get("/api/v1/space/123kasdk01", {
      headers: {
        authorization: `Bearer ${userToken}`,
      },
    });
  ...
  test("Correct spaceId returns all the elements", async () => {
    const response = await axios.get(`/api/v1/space/${spaceId}`, {
      headers: {
        authorization: `Bearer ${userToken}`,
      },
    });
  ...
  test("Delete endpoint is able to delete an element", async () => {
    ...
    const res = await axios.delete("/api/v1/space/element", {
    ...
    const newResponse = await axios.get(`/api/v1/space/${spaceId}`, {
      headers: {
        authorization: `Bearer ${userToken}`,
      },
    });
...
describe("Admin Endpoints", () => {
  ...
  test("Admin is able to update the imageUrl for an element", async () => {
    ...
    const updateElementResponse = await axios.put(
      `/api/v1/admin/element/${elementResponse.data.id}`,
      {
        imageUrl:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQm3RFDZM21teuCMFYx_AROjt-AzUwDBROFww&s",
      },
...
describe("Websocket tests", () => {
  ...
  async function setupHTTP() {
    const username = `rohit-${Math.random()}`;
    const password = "123456";
    const adminSignupResponse = await axios.post("/api/v1/signup", {
      username,
      password,
      type: "admin",
    });

    const adminSigninResponse = await axios.post("/api/v1/signin", {
      username,
      password,
    });
