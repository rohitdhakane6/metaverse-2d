...
export const CreateMapSchema = z.object({
  ...
  dimensions: z
    ...
    .regex(
      /^[0-9]{1,4}x[0-9]{1,4}$/,
      "Invalid dimensions format (should be WxH, e.g., 500x500)"
    ),
  ...
  defaultElements: z.array(
    z.object({
      ...
      x: z
        ...
        .max(10000, "X coordinate is too large"), // Example bounds for X
      y: z
        .number()
        .min(0, "Y coordinate cannot be negative")
        .max(10000, "Y coordinate is too large"), // Example bounds for Y
    })
  ),
});
